...
{
	F.open_chunk	(EEditableMeshChunks::EMESH_CHUNK_VERSION);
	F.w_u16       	((u16)EEditableMeshVersions::EMESH_CURRENT_VERSION);
	F.close_chunk  	();

	F.open_chunk	(EEditableMeshChunks::EMESH_CHUNK_MESHNAME);
    F.w_stringZ		(m_Name);
	F.close_chunk   ();

	F.w_chunk		(EEditableMeshChunks::EMESH_CHUNK_BBOX,&m_Box, sizeof(m_Box));
	F.w_chunk		(EEditableMeshChunks::EMESH_CHUNK_FLAGS,&m_Flags,1);
	F.w_chunk		(EEditableMeshChunks::EMESH_CHUNK_BOP,&m_Ops, sizeof(m_Ops));

	F.open_chunk	(EEditableMeshChunks::EMESH_CHUNK_VERTS);
	F.w_u32			(m_VertCount);
    F.w				(m_Vertices, m_VertCount*sizeof(Fvector));

	F.close_chunk     ();

	F.open_chunk	(EEditableMeshChunks::EMESH_CHUNK_FACES);
	F.w_u32			(m_FaceCount);
    ...
    if (GetSmoothGroups())
    {
        F.open_chunk	(EEditableMeshChunks::EMESH_CHUNK_SG);
	...
	if (m_Normals)
	{
		F.open_chunk(EEditableMeshChunks::EMESH_CHUNK_NORMALS);
		F.w(m_Normals, m_FaceCount * 3 * sizeof(Fvector));
		F.close_chunk();
	}

	F.open_chunk	(EEditableMeshChunks::EMESH_CHUNK_VMREFS);
	...
	F.open_chunk	(EEditableMeshChunks::EMESH_CHUNK_SFACE);
	...
	F.open_chunk	(EEditableMeshChunks::EMESH_CHUNK_VMAPS_2);
...
bool CEditableMesh::LoadMesh(IReader& F){
    ...
    if (version!=(u32)EEditableMeshVersions::EMESH_CURRENT_VERSION){
        ELog.DlgMsg( mtError, "CEditableMesh: unsuported file version. Mesh can't load.");
        return false;
    }

    R_ASSERT(F.find_chunk(EEditableMeshChunks::EMESH_CHUNK_MESHNAME));
	F.r_stringZ		(m_Name);

    R_ASSERT(F.r_chunk(EEditableMeshChunks::EMESH_CHUNK_BBOX,&m_Box));
    R_ASSERT(F.r_chunk(EEditableMeshChunks::EMESH_CHUNK_FLAGS,&m_Flags));
    F.r_chunk(EEditableMeshChunks::EMESH_CHUNK_BOP,&m_Ops);

    R_ASSERT(F.find_chunk(EEditableMeshChunks::EMESH_CHUNK_VERTS));
	m_VertCount			= F.r_u32();
    if (m_VertCount<3){
    ...
    R_ASSERT(F.find_chunk(EEditableMeshChunks::EMESH_CHUNK_FACES));
    m_FaceCount			= F.r_u32();
	...
	u32 sg_chunk_size	= F.find_chunk(EEditableMeshChunks::EMESH_CHUNK_SG);
	if (sg_chunk_size){
		VERIFY			(m_FaceCount*sizeof(u32)==sg_chunk_size);
		F.r				(m_SmoothGroups, m_FaceCount*sizeof(u32));
	}

	u32 normal_chunk_size = F.find_chunk(EEditableMeshChunks::EMESH_CHUNK_NORMALS);
    ...
    R_ASSERT(F.find_chunk(EEditableMeshChunks::EMESH_CHUNK_VMREFS));
    ...
    R_ASSERT(F.find_chunk(EEditableMeshChunks::EMESH_CHUNK_SFACE));
    string128 surf_name;
    ...
    if(F.find_chunk(EEditableMeshChunks::EMESH_CHUNK_VMAPS_2))
	{
	...
	{
		if(F.find_chunk(EEditableMeshChunks::EMESH_CHUNK_VMAPS_1))
		{
			m_VMaps.resize	(F.r_u32());
			for (VMapIt vm_it=m_VMaps.begin(); vm_it!=m_VMaps.end(); vm_it++)
			{
		...
		{
			R_ASSERT(F.find_chunk(EEditableMeshChunks::EMESH_CHUNK_VMAPS_0));
			m_VMaps.resize	(F.r_u32());
			for (VMapIt vm_it=m_VMaps.begin(); vm_it!=m_VMaps.end(); vm_it++)
			{
				*vm_it		= new st_VMap();
