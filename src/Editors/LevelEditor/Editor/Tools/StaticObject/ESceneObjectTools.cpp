...
ESceneObjectTool::ESceneObjectTool() :
    ESceneCustomOTool(OBJCLASS_SCENEOBJECT)
{
        ...
        [&](CCustomObject* ObjPtr)
        {
            ...
            for (ObjectIt b_it = m_Objects.begin(); b_it != m_Objects.end(); b_it++)
            {
                ...
                if (A->RefCompare(B->GetReference()))
                {
                    ...
                    {
                        auto MessageType = mtWarning;
                        if (EPrefs->DuplicateObjectPositionAsError)
                        {
                            bRes = false;
                            MessageType = mtError;
                        }
                        ELog.Msg(MessageType, "Duplicate object position '%s'-'%s' with reference '%s'.", A->GetName(), B->GetName(), A->RefName());
                    }
                }
            }
...
void ESceneObjectTool::FillAppendRandomPropertiesBegin(UIPropertiesForm& Prop)
{
    ...
    PropValue* V;
    PropItemVec 					items;
    V = PHelper().CreateCaption(items, "Scale", "");
    ...
    V = PHelper().CreateCaption(items, "Rotate", "");
    V->OnChangeEvent.bind(this, &ESceneObjectTool::OnChangeAppendRandomFlags);
