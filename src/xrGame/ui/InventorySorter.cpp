...
void CInventorySorter::LoadCategoryFromXml(const shared_str& categoryId, EInventorySortCategory category)
{
    ...
    info._name = pSettings->read_if_exists<str_c>("inventory_sort_categories", namePath, info._name.c_str());
    info._hint = pSettings->read_if_exists<str_c>("inventory_sort_categories", hintPath, info._hint.c_str());
    
    shared_str iconTexture = pSettings->read_if_exists<str_c>("inventory_sort_categories",iconPath,nullptr);
