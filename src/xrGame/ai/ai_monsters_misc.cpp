...
u32 dwfChooseAction(u32 dwActionRefreshRate, float fMinProbability0, float fMinProbability1, float fMinProbability2, float fMinProbability3, u32 dwTeam, u32 dwSquad, u32 dwGroup, u32 a0, u32 a1, u32 a2, u32 a3, u32 a4, CEntity *tpEntity, float fGroupDistance)
{
	...
	if (!tpEntity)
	{
		...
		{
			if (entity && entity->g_Alive() && (entity->SpatialComponent->spatial.type & ESPATIAL_TYPE::VISIBLEFORAI) != ESPATIAL_TYPE::NONE)
	...
	else
	{
		for (CEntity* entity : Group.members())
		{
			if (entity && entity->m_entity_condition && entity->g_Alive() && (entity->SpatialComponent->spatial.type & ESPATIAL_TYPE::VISIBLEFORAI) != ESPATIAL_TYPE::NONE)
			{
...
void CAniVector::Load(IKinematicsAnimated* tpKinematics, const char* caBaseName)
{
	A.clear();
	string256	S1, S2;
