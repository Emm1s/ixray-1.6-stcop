...
void CZombie::StartFakeDeathRagdoll()
{
	...
	IsSpatialReactSound = (SpatialComponent->spatial.type & ESPATIAL_TYPE::REACTTOSOUND) != ESPATIAL_TYPE::NONE;
	IsSpatialPhysMove = (SpatialComponent->spatial.type & ESPATIAL_TYPE::PHYSIC_MOVEMENT) != ESPATIAL_TYPE::NONE;
...
void CZombie::shedule_Update(u32 dt)
{
	...
	if (g_Alive() && time_dead_start != 0) {
		...
		if (EngineExternal()[EEngineExternalGame::EnableRagdolledZombiePseudodeath])
		{
			...
			CanWakeUp = !CameraFrustum.testSphere_dirty(SpatialComponent->spatial.sphere.P, SpatialComponent->spatial.sphere.R);
			if (CanWakeUp)
			{
				if (!time_out_frustum)
				{
