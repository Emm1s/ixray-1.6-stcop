...
void CPoltergeist::Load(const char* section)
{
	...
	{
		m_poltergeist = new CFlamePoltergeist(this);
		m_poltergeist->load(section);
	}
	else
	{
		m_poltergeist = new CTelekineticPoltergeist(this);
		m_poltergeist->load(section);
	}
...
void CPoltergeist::update_detection()
{
	...
	const float dist2actor = actor_pos.distance_to(Position());

	const float time_passed_sec = float(Device.dwTimeGlobal - m_last_detection_time) / 1000.f;
	...
	{
		const float relative_range = dist2actor / get_detection_far_range();
		const float range_factor = relative_range * get_detection_far_range_factor() +
			(1.f - relative_range) * get_detection_near_range_factor();

		const float speed_factor = get_detection_speed_factor();
		const float raw_speed = m_last_actor_pos.distance_to(actor_pos) / time_passed_sec;

		const float speed = powf(1.f + raw_speed, speed_factor) - 1.f;

		const float actor_psy_immunity = Actor()->conditions().GetHitImmunity(ALife::eHitTypeTelepatic);
...
{
	if (m_poltergeist && m_enable_corpse_on_death)
	{
		if (state_invisible)
		{
			setVisible(true);
