...
void CWeapon::Load		(const char* section)
{
	...
	u8 rm = pSettings->read_if_exists<u8>(section, "cam_return", 1 );
	...
	cam_recoil.DispersionFrac	= std::abs( pSettings->read_if_exists<float>(section, "cam_dispersion_frac", 0.7f ) );
