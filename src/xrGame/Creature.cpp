// CustomMonster.cpp: implementation of the CCreature class.
//
//////////////////////////////////////////////////////////////////////
...
void CCreature::mk_orientation(Fvector &dir, Fmatrix& mR)
{
...
void CCreature::net_Export(NET_Packet& P)					// export to server
{
	R_ASSERT				(Local());
...
void CCreature::net_Import(NET_Packet& P)
{
	R_ASSERT				(Remote());
	net_update				N;
...
{
	PROF_EVENT("CCreature::shedule_Update");
...
void CCreature::net_update::lerp(CCreature::net_update& A, CCreature::net_update& B, float f)
{
	// 
...
void CCreature::update_range_fov	(float &new_range, float &new_fov, float start_range, float start_fov)
{
...
void CCreature::HitSignal(float /**perc/**/, Fvector& /**vLocalDir/**/, CObject* /**who/**/)
{
}

void CCreature::Die	(CObject* who)
{
	inherited::Die			(who);
...
#ifdef DEBUG_DRAW
void CCreature::OnHUDDraw(CCustomHUD *hud)
{
}
#endif

void CCreature::Exec_Action(float /**dt/**/)
{
}
...
bool CCreature::is_special_killer(CObject *obj)
{
...
bool CCreature::useful			(const CItemManager *manager, const CGameObject *object) const
{
	return	(memory().item().useful(object));
}

float CCreature::evaluate		(const CItemManager *manager, const CGameObject *object) const
{
	return	(memory().item().evaluate(object));
}

bool CCreature::useful			(const CEnemyManager *manager, const CEntityAlive *object) const
{
	return	(memory().enemy().useful(object));
}

float CCreature::evaluate		(const CEnemyManager *manager, const CEntityAlive *object) const
{
	return	(memory().enemy().evaluate(object));
}

bool CCreature::useful			(const CDangerManager *manager, const CDangerObject &object) const
{
	return	(memory().danger().useful(object));
}

float CCreature::evaluate		(const CDangerManager *manager, const CDangerObject &object) const
{
	return	(memory().danger().evaluate(object));
}

CMovementManager *CCreature::create_movement_manager	()
{
	return	(new CMovementManager(this));
}

CSound_UserDataVisitor *CCreature::create_sound_visitor		()
{
	return	(m_sound_user_data_visitor	= new CSound_UserDataVisitor());
}

CMemoryManager *CCreature::create_memory_manager		()
{
	return	(new CMemoryManager(this,create_sound_visitor()));
}

const SRotation CCreature::Orientation	() const
{
	return					(movement().m_body.current);
};

const MonsterSpace::SBoneRotation &CCreature::head_orientation	() const
{
	return					(movement().m_body);
}

DLL_Pure *CCreature::_construct()
{
...
void CCreature::net_Relcase	(CObject *object)
{
	inherited::net_Relcase		(object);
	memory().remove_links		(object);
}

void CCreature::set_fov		(float new_fov)
{
	VERIFY		(new_fov > 0.f);
	eye_fov		= new_fov;
}	

void CCreature::set_range		(float new_range)
{
	VERIFY		(new_range > 1.f);
	eye_range	= new_range;
}

void CCreature::on_restrictions_change	()
{
...
void CCreature::on_enemy_change(const CEntityAlive *enemy)
{
}

CVisualMemoryManager *CCreature::visual_memory	() const
{
	return						(&memory().visual());
}

void CCreature::save (NET_Packet &packet)
{
...
void CCreature::load (IReader &packet)		
{
...
bool CCreature::update_critical_wounded	(const u16 &bone_id, const float &power)
{
...
Fvector CCreature::predict_position	(const float &time_to_check) const
{
	return							(movement().predict_position(time_to_check));
}

Fvector CCreature::target_position	() const
{
	return							(movement().target_position());
}

void CCreature::create_anim_mov_ctrl	( CBlend *b, Fmatrix *start_pose, bool local_animation  )
{
...
void CCreature::destroy_anim_mov_ctrl	()
{
...
void CCreature::ForceTransform(const Fmatrix& m)
{
