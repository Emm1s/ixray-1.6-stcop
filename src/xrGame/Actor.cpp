...
void CActor::UpdateCL()
{
	...
	if(g_Alive() && Level().CurrentViewEntity() == this)
	{
		if(CurrentGameUI() && nullptr==CurrentGameUI()->TopInputReceiver())
		{
			...
			{
				pPickup->UpdateClPickup(bUseInteraction);
			} else if (bUseInteraction)
			{
				pPickup->SetPickupMode(true);
			}
		}
	...
	{
		PROF_EVENT("CActor UpdateCL - PickupUpdate");
		if (g_Alive())
		{
			PickupModeUpdate();
		}
...
//			Msg( "bs            = %.2f", bs );
	...
	if (!input_external_handler_installed() && LookAtData.LookAtObject && LookAtData.IsNearEnough /*&& RQ.O && RQ.O->getVisible() && ActorPos.distance_to_sqr(PickPos) < 6.0f*/)
	{
