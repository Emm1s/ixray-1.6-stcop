...
void	export_geometry		( xrMU_Model &	mu_model )
{
	...
	for (xrMU_Model::v_subdivs_it it=mu_model.m_subdivs.begin(); it!=mu_model.m_subdivs.end(); it++)
	{
		// Vertices
		{
			...
			VB->End(&it->vb_id,&it->vb_start);
		}

		// Indices
		IB->Register((u16*)(it->ogf->data.faces.data()),(u16*)(&*it->ogf->data.faces.end()),&it->ib_id,&it->ib_start);

		// SW
		if (it->ogf->progressive_test())
		{
			SWI->Register(&it->sw_id,&it->ogf->data.m_SWI);
		}
	}
}
