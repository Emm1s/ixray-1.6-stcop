...
namespace
{
	struct RedImageMemoryWriteContext
	{
		u8* Buffer;
		size_t Capacity;
		size_t Size;
	...
	void RedImageMemoryWrite(void* context, void* data, int size)
	{
		RedImageMemoryWriteContext* writeContext = static_cast<RedImageMemoryWriteContext*>(context);
		const size_t writeSize = static_cast<size_t>(size);

		if (writeContext->Size + writeSize <= writeContext->Capacity)
		{
			memcpy(writeContext->Buffer + writeContext->Size, data, writeSize);
		}
		else
		{
			writeContext->Overflow = true;
		}

		writeContext->Size += writeSize;
	}
}

bool RedImage::SaveToJpgMemory(void* buffer, size_t capacity, size_t& size, int quality, size_t depth)
{
	assert(m_Depth > depth);
	assert(!RedTextureUtils::isCompressor(m_PixelFotmat));
	assert(!RedTextureUtils::isFloatPixel(m_PixelFotmat));
	...
	RedImageMemoryWriteContext context = {};
	context.Buffer = static_cast<u8*>(buffer);
	context.Capacity = capacity;

	const int result = stbi_write_jpg_to_func(
		RedImageMemoryWrite,
		&context,
		static_cast<int>(m_Width),
		static_cast<int>(m_Height),
		static_cast<int>(RedTextureUtils::GetCountComp(m_PixelFotmat)),
		m_ImageBuffer + depth * RedTextureUtils::GetSizeInMemory(m_Width, m_Height, m_Mips, m_PixelFotmat),
		quality
	);

	size = context.Size;
	return result != 0 && !context.Overflow;
}
