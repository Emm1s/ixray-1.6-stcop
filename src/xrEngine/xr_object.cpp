...
void CObject::Load				(const char* section )
{
	...
	SpatialComponent->spatial.ssa_dyn_factor = pSettings->read_if_exists<float>(section, "ssa_dyn_factor", 0.002f);// минимальный размер на экране при котором объект еще будет виден
	SpatialComponent->spatial.ssa_d_cam = pSettings->read_if_exists<float>(section, "ssa_d_cam", 220.f);//дистанция в совокупности с fov на которой еще видно объект
}
